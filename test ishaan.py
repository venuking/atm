...
def dbcon():
    ...
    try:
        con=myc.connect (host='Local host',user='root',password='psd')
        mycursor=con.cursor()
        mycursor.execute('DROP DATABASE IF EXSISTS ATM')
        mycursor.execute('CREATE DATABASE ATM')
        mycursor.execute('USE ATM')
        mycursor.execute('DROP TABLE IF EXSISTS RECORDS')
        sql=" CREATE TABLE RECORDS(ACCOUNT_NO  INT(25) PRIMARY KEY, PASSWORD  VARCHAR(25) NOT NULL, NAME VARCHAR(25)NOT NULL)"
...
conn=sql.connect(host='localhost',user='root',password='psd',database='  ATM_MACHINE')
c1=conn.cursor()
print("================================================================================")
